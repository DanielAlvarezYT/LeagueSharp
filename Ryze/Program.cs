...
#pragma warning disable CS0618 // Type or member is obsolete
                Utility.DrawCircle(Player.Position, _rangeR, System.Drawing.Color.DeepPink, 2, 45, true);
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void Obj_AI_Base_OnLevelUp(Obj_AI_Base sender, EventArgs args)
        {
            ...
            if ((_lvl2 == _lvl3) || (_lvl2 == _lvl4) || (_lvl3 == _lvl4))
                return;
            var delay = 700;
            Utility.DelayAction.Add(delay, () => LevelUp(_lvl1));
            Utility.DelayAction.Add(delay + 50, () => LevelUp(_lvl2));
            Utility.DelayAction.Add(delay + 100, () => LevelUp(_lvl3));
            Utility.DelayAction.Add(delay + 150, () => LevelUp(_lvl4));
        }
        ...
        private static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            if (!_menu.Item("WGapCloser").GetValue<bool>() ||
                (Player.Mana < _w.Instance.ManaCost + _q.Instance.ManaCost))
                return;
        ...
        private static void RZhonya()
        {
            if ((Player.HealthPercent < 50) && _menu.Item("UseRAndZhonyaEscape").GetValue<bool>() &&
                ...
                if (Zhonya.IsReady())
                {
                    ...
                    Utility.DelayAction.Add(145, delegate
                    {
        ...
        {
            float damage = 0;
            if (_q.IsReady() || (Player.Mana <= _q.Instance.ManaCost + _e.Instance.ManaCost))
                damage += QGetRealDamage(enemy);
            else if (_q.IsReady() || (Player.Mana <= _q.Instance.ManaCost))
                damage += QDefaultDamage(enemy);

            if (_w.IsReady() || (Player.Mana <= _w.Instance.ManaCost + _w.Instance.ManaCost))
                damage += _w.GetDamage(enemy) + _w.GetDamage(enemy);
            else if (_w.IsReady() || (Player.Mana <= _w.Instance.ManaCost))
                damage += _w.GetDamage(enemy);

            if (_e.IsReady() || (Player.Mana <= _e.Instance.ManaCost + _e.Instance.ManaCost))
                damage += _e.GetDamage(enemy) + _e.GetDamage(enemy);
            else if (_e.IsReady() || (Player.Mana <= _e.Instance.ManaCost))
